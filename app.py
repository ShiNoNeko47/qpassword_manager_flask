...
def register():
    taken_usernames = []
    for username in filter(
            lambda key: not key.isdigit(), redis.keys("*")):
    ...
    return "Please confirm your email to finish the registration" + f": http://localhost:5000/confirm_email/{username}/{code}"
    ...
    try:
        ...
        if account[0] == username:
            redis.json().set(
                f"{account[0]}",
                "$",
                {"master_key": account[1], "email": account[2],
                    "next_id": 0, "passwords": {}}
            )
            redis.delete(code)
...
def add_to_database():
    if request.authorization:
        if check_credentials():
            ...
            redis.json().set(
                username, f"$.passwords.{entry_id}", request.json)
